...
<div class="row mb-3">
    ...
    <div class="col-md-2 col-sm-12">
        <div class="director-kpi-card">
            ...
            <div class="director-kpi-value">{{ $resultSummary['coverage_rate'] ?? 0 }}%</div>
   ...
   $(document).ready(function () {
      ...
      stateSave: true,
       ...
       drawCallback: function () {
            ...
            var visibleTotal = rows.length;
            var visibleConfirmed = 0;
            var visiblePending = 0;
            var visiblePink = 0;

            for (var i = 0; i < visibleTotal; i++) {
                if (parseInt(rows[i].verify_by_constituency, 10) === 1) {
                    visibleConfirmed++;
                } else {
                    visiblePending++;
                }
                if (rows[i].pink_sheet_path) {
                    visiblePink++;
                }
            }

            if ($('#election_type_id').val() !== 'all') {
                $('#summary_submitted').text(visibleTotal.toLocaleString());
                $('#summary_confirmed').text(visibleConfirmed.toLocaleString());
                $('#summary_pending').text(visiblePending.toLocaleString());
                $('#summary_pink').text(visiblePink.toLocaleString());
            } else {
                $('#summary_submitted').text(Number($('#summary_submitted').data('default') || 0).toLocaleString());
                $('#summary_confirmed').text(Number($('#summary_confirmed').data('default') || 0).toLocaleString());
                $('#summary_pending').text(Number($('#summary_pending').data('default') || 0).toLocaleString());
                $('#summary_pink').text(Number($('#summary_pink').data('default') || 0).toLocaleString());
            }
       }
       });
