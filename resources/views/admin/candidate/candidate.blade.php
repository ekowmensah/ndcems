...
<div class="container">
    <div class="row">
        ...
        <div class="col-md-9">
            <br>
            @if($type)
                    <a href="{{route('SuperAdmin.candidateRegister',$type->id)}}"  style=" float:  right;" class="btn btn-success">Add {{$type->name}} Candidate</a>

            @else
                @foreach ($_electionTypes as $electionType)
                    <a href="{{route('SuperAdmin.candidateRegister',$electionType->id)}}"  style=" float:  right;" class="btn btn-success">Add {{$electionType->name}} Candidate</a>
                @endforeach
            @endif
            </div>
    </div>
